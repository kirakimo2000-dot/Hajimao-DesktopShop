...
- SQLite schema v5 逐级兼容迁移旧 v1/v2/v3/v4 存档，并完整保存采购与员工经营状态。
...
0.1.4 继续优先完成 Domain/Application/Infrastructure 底层。采购和员工命令、不可变快照与完整存档已可供后续管理前端接入，但当前 WPF 仍运行原型单店兼容适配器；正式经营页面仍随 0.1.6 管理前端制作。
