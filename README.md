...
- 正式场景使用一张 256×256 嵌入式像素图集，包含收银员、补货员、顾客四帧动作、三类货架和十种商品。
- Windows 减少动态效果开启时角色固定首帧；管理窗以 250 ms 更新表现，仅桌面放置时降至 2 秒刷新。
- 八类短促芯片提示音由程序确定性生成并受 16 KiB 单音预算约束；托盘使用 32×32 品牌像素店面图标。

0.1.7 已完成正式 Sprite、角色动作、定制音效和品牌托盘身份，同时保持玩法、存档和固定现实 1x 不变。0.1.8 将进入商业街 Beta，补齐多店共享街道可视化与环境层。
