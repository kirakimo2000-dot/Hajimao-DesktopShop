...
- SQLite schema v3 兼容迁移旧 v1/v2 单店存档，不删除旧商品、库存、资金或现场字段。
...
0.3.0 继续优先完成 Domain/Application/Infrastructure 底层。完整多店会话已经可以写入 SQLite，但当前 WPF 仍运行原型单店兼容适配器，不把底层完成误报为正式管理前端已接入；正式管理前端仍计划在 0.7.0 制作。
