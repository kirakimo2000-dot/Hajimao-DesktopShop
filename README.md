...
## 当前版本：0.1.29 Playable Demo Candidate 1

- 新玩家从社区便利、平价量贩、精品食品三种首店中选择开局；界面只解释赚钱方式和风险，后台应用真实经营形态数值。
- 旧存档直接恢复，不重复选择；首店身份会随 schema v7 存档保存，关闭程序期间仍完全冻结。
- 具体店铺页先给出“本店表现、主要原因、下一项投资”三段结论；完整财务、事件和投资算法继续在后台计算。
- 当前市场事件并入经营原因，最近投资的完整日结对比并入下一项投资，不再把店铺组合和六格会计报表堆给玩家。
- 管理窗采用统一的暖色深色像素设计令牌，普通文字、弱化文字、主按钮和焦点框均有自动对比度门禁。
- 主导航和关键操作保留至少 44 像素命中高度；右栏先显示唯一下一步，再显示可选对象详情。

- 管理窗口只保留经营概览、经营策略和投资决策三个界面；内部模块不再直接变成玩家的操作菜单。
- 全店定价提供高周转、均衡和高毛利定位；库存投入提供精益、均衡和充足备货，应用后由自动运营持续执行。
- 右栏只显示一个“下一步”：三步真实动作闭环完成后自动切换到当前长期目标，不再同时堆叠任务和说明。
...
- 右侧长期目标从真实日结、共享现金、已开店铺与各店成长快照推导；目标会从盈利、首次投资延伸到下一家店和持续强化最弱店铺。
...
- 玩家累计经验决定等级并解锁商品；开新店由资金与品牌选择决定，开局只有一家店。
...
- SQLite schema v7 逐级兼容迁移旧 v1～v6 存档，并完整保存店铺品牌/形态、采购、员工、成长与活动状态。
...
- 多家店共享同一街道人流，访客机会随实际店铺数量增长并按吸引力分配；开设新店提高街道总客流但也形成店间竞争。
- 晴朗、多云、雨天和大风按经营时间确定性变化，并影响共享客流；道路同步显示受预算约束的路人和车辆。
- 商业街页面显示逐店吸引力与客流份额；经营页移除底部状态栏和固定 1x 展示文字。
- 三步无状态新手任务直接从策略、投资和回报证据推导；概览默认可见，不再用运行 1 Tick 伪装“已观察”。
...
0.1.24 在品牌与经营形态底层之上加入 32 个员工档案、32 个市场事件、确定性事件调度和存档恢复；事件只随游戏实际运行推进，并通过既有需求模型影响客流、购买率与品类偏好。自动策略和端到端测试不能替代人类理解证据；完成三轮真实陌生玩家测试前，项目不会把 Candidate 写成正式 Demo 1。

后续请求发布时，便携 ZIP、校验和与发布 JSON 将出现在 [GitHub Releases](https://github.com/kirakimo2000-dot/Hajimao-DesktopShop/releases)；公开测试资产只应列出这些便携资产。
