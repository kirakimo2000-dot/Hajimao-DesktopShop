...
- 2026-08-06：人物“动画帧数”和“图片关键姿势数”必须分开建模。统一 24 帧时间轴可让路线每帧推进，同时用 8 个关键姿势×3 帧去重播放，避免把相同 PNG 像素存三遍；若未来要求 24 张独立手绘姿势，应作为单独美术资产阶段。
- 2026-08-06：原 256×256 RGBA 图集的收银员第 5～7 格包含 13、17、5 像素的独立连通块；顾客手提袋仍与主体连通。四邻域连通审计比仅检查“非空帧”更能捕获生成式 Sprite 的断肢/漂浮噪点。
- 2026-08-06：无抖动 256 色索引 PNG 在最近邻 4 倍预览下保持像素轮廓，并把正式图集从 57,191 字节降到 16,623 字节；运行时再审计可防止构建工具被绕过后把坏格嵌入程序集。
