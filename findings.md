...
- 2026-08-05：用户明确“街区内容不能有横向上限”，因此窗口宽度与内容宽度必须分离：内容公式为 `24 + N×224 + (N-1)×8`，窗口只夹取到当前显示器工作区，镜头负责剩余内容。
- 2026-08-05：人物换帧与位置变化是两个独立问题；原表现计数每八帧归零会让长路径永远无法前进，现改为连续演示时钟，由 `CharacterMotion` 分别计算帧、循环路径和往返路径。
- 2026-08-05：与桌面原始需求相比，本次已补齐街区常驻、店面进入、多店连续增长、路人/车辆/天气和八帧移动；仍未完成逐货架拿货/结账动作、货架/员工直接点击、参考图级定制店面美术与 Authenticode 签名。
- 2026-08-05：Windows PowerShell 5.1 的 `ProcessStartInfo` 没有 `ArgumentList`，旧兼容分支对所有参数加引号；msiexec 不接受被引用成值的 `/a`、`/qn` 开关。兼容引用函数必须原样保留无空白开关，只对含空白的路径/属性值加引号并处理末尾反斜杠。
- 2026-08-05：自包含 ZIP 与 MSI 即使关闭 SourceRevision 信息，也会因压缩/安装包时间元数据在重建时产生不同 SHA；GitHub Release 必须使用合并态重建清单中的哈希，不能沿用功能分支验证构建的哈希。
