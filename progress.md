...
- 2026-08-05：开始 0.1.10 桌面街区导航；按用户最新约束将固定街区宽度改为无上限内容宽度，初始单店为 248×180，超出工作区才启用镜头。
- 2026-08-05：完成街区/具体店铺导航状态、店面命中、滚轮镜头、动态任务栏上方停靠和托盘回街区；经营状态与页面切换保持分离。
- 2026-08-05：通过正式 Sprite 管线将收银员、补货员、顾客扩展至八帧，保留既有首帧与货架/商品；店内人物、街区行人/车辆产生真实位置变化，减少动态效果时冻结帧和位置。
- 2026-08-05：0.1.10 本地门禁通过：Release 401/401、构建 0 警告/0 错误、11 项目无已知直接/传递漏洞；隔离真实进程响应正常并创建 SQLite/日志。存档 schema v6、固定现实 1x 与离线同管线均未改变。
- 2026-08-05：发布冒烟的 PS5.1 原生参数兼容分支被精确复现：错误地引用 `/a`、`/qn` 会令 msiexec 等待至超时。先新增 Release RED 契约再修复，仅含空白参数使用引号；0.1.10 便携版和 MSI administrative image 完整冒烟随后通过。
- 2026-08-05：PR #6 在统一 WPF STA 冷启动门限后由 Actions run `30975643419` 通过管理员完整 MSI 生命周期，并 squash 合并为 `50053a8`。合并态重新生成并验收 0.1.10 发布物，最终 ZIP/MSI SHA-256 分别为 `8b4d4292...fab12` 与 `067ad261...a9e12`。
- 2026-08-05：正式标签与 GitHub Release `v0.1.10` 已发布，四个资产的名称、字节数和摘要已复核；本地标签已同步到远端标签对象 `28a2df3`（提交 `35f163b`）。生产目录与功能分支逐项比对无差异后，隔离工作树及本地/远端功能分支均已清理，主工作区仅保留正式发布物。
- 2026-08-05：按用户要求清理本地构建包；删除原型/旧正式版本的四组解包目录与 ZIP、空 `staging` 和本地 `release/0.1.9`，约释放 1.7 GB，仅保留 `artifacts/release/0.1.10` 正式文件。历史源码与 GitHub Release 不受影响。
- 2026-08-05：开始 0.1.11 顾客分段行为；建立隔离分支 `agent/v0.1.11-customer-behavior`，基线 Release 401/401 通过。计划保存于 `docs/superpowers/plans/2026-08-05-v0.1.11-customer-journeys.md`。
- 2026-08-05：路径测试先因 `Rendering.Customers` 类型不存在得到编译 RED，再完成进店、找货架、拿货、排队、结账、离店六阶段，以及常温/冷藏/冷冻落点和减少动态效果冻结，聚焦 11/11 GREEN。
- 2026-08-05：渲染测试以零排队访客像素哈希不变得到 2 项预期 RED；接入后访客按稳定商品顺序走向对应货架并显示拿货标记，旅程与排队共享最多 5 人预算，聚焦 7/7、Rendering 57/57 GREEN。
- 2026-08-05：版本契约先以期望 0.1.11、实际 0.1.10 得到 RED，提升补丁版本后 GREEN；本地完整门禁 414/414、Release 构建 0 警告/0 错误，11 项目无已知直接/传递 NuGet 漏洞。schema v6、经营公式、固定现实 1x 与窗口行为保持不变。
- 2026-08-05：0.1.11 本地发布物和冒烟完成；ZIP 88,282,986 字节、SHA-256 `954d0cee...c5086`，MSI 74,325,708 字节、SHA-256 `193dd021...cd6a3`。非管理员 MSI administrative image 验证程序响应、SQLite、日志、任务栏隐藏窗口与安全清理，完整 per-machine 安装仍由 GitHub Windows 管理员门禁执行。
- 2026-08-05：因 `github.com:443` 区域路由连续失败，15 个变更文件通过 Git Data API 逐 blob 上传且每项 SHA 与本地 Git blob 完全一致；PR #7 的 Actions run `30981688835` 随后在管理员 runner 完成完整 MSI 生命周期并通过，PR squash 合并为 `78820fa`。合并态再次生成相同字节数的 0.1.11 发布物，正式 ZIP/MSI SHA-256 为 `de8fe236...aa480a` 与 `0aba0334...aaaf4`，本地冒烟通过。
- 2026-08-05：注释标签和 GitHub Release `v0.1.11` 已发布，四个远端资产的名称、字节数与本地主分支摘要完全一致；本地标签对象同步为 `5f992b5`，解引用提交 `47a8efa`。旧本地 `release/0.1.10`、发布 staging、所有生成的 `bin/obj`、隔离工作树及本地/远端功能分支均已清理，主工作区只保留 `release/0.1.11` 四个正式文件。
- 2026-08-06：0.1.14 将店内与街区全部人物统一到 24 帧表现时间轴，以 8 个互不重复关键姿势去重存储；人物位置继续逐帧推进，固定现实 1x、schema v6 和经营规则不变。
- 2026-08-06：图像审计精确发现收银员第 5～7 格的 13/17/5 像素断裂噪点；确定性清理 35 像素后，全部 24 个已存人物格通过非空、单连通、边界、锚点和唯一性检查。
- 2026-08-06：运行时图集从 57,191 字节压缩至 16,623 字节（减少 70.9%），硬预算收紧到 24 KiB；Python 工具 4/4、C# 462/462、Release 构建 0 警告/0 错误，便携程序隔离启动并响应正常。
