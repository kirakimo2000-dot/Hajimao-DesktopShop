...
- 2026-08-05：因 `github.com:443` 区域路由连续失败，15 个变更文件通过 Git Data API 逐 blob 上传且每项 SHA 与本地 Git blob 完全一致；PR #7 的 Actions run `30981688835` 随后在管理员 runner 完成完整 MSI 生命周期并通过，PR squash 合并为 `78820fa`。合并态再次生成相同字节数和 SHA 的 0.1.11 发布物，本地冒烟通过。
