...
public sealed class BusinessDesktopShopRenderer : IDisposable
{
    public const int LogicalWidth = 420;
    public const int LogicalHeight = 280;

    private readonly BusinessShopSceneRenderer _sceneRenderer = new();
    ...
    public BusinessDesktopShopRenderer()
    {
        _font = new SKFont(_monoTypeface, 14f);
        _smallFont = new SKFont(_typeface, 12f);
    }

    public void Render(SKCanvas canvas, SKImageInfo target, BusinessShopFrame frame)
    {
        ...
        Text(canvas, frame.GameTimeText, 238, 26, "#F4EBDD", _smallFont);
        Button(canvas, 307, 5, 48, 34, frame.IsLocked ? "解锁" : "锁定", false);
        Button(canvas, 361, 5, 49, 34, "经营", true);
        canvas.Save();
        canvas.Translate(0, 50);
        _sceneRenderer.DrawLogicalScene(canvas, frame.Scene);
        canvas.Restore();
        Fill(canvas, 7, 250, 4, 18, "#E15A5A");
        Text(canvas, frame.StockWarningText, 16, 264, "#E15A5A", _smallFont);
