...
0.2.5 店铺切换永久卡死修复已完成代码验证；完成最终单文件实机验收后，下一阶段回到 0.2.x 的玩法表现、引导与 UI/UX 完善。
...
- [x] 0.1.8 商业街 Beta
- [x] 0.1.9 发布候选与长时验收
- [x] 0.1.10 无限街区、街区/店铺导航和八帧人物移动
- **Status:** complete

### Phase 17: 0.1.10 桌面街区导航

- [x] 街区内容宽度不设上限，初始一家店只占一个店面宽度
- [x] 桌面街区与具体店铺双页面、店面点击、返回街区和托盘恢复
- [x] 动态任务栏上方窗口、工作区宽度夹取和滚轮镜头
- [x] 三类人物八帧图集、连续表现时钟、店内与街区实际位移
- [x] Release 全量测试、构建、漏洞审计和真实隔离启动
- [x] GitHub PR、Windows 管理员门禁与 squash 合并
- [x] v0.1.10 标签/Release 与隔离工作区清理
- **Status:** complete

### Phase 18: 0.1.11 顾客分段行为

- [x] 进店、寻找货架、拿货、加入队列、结账和离店六阶段纯路径
- [x] 常温、冷藏、冷冻三类货架目的地与连续整数像素位置
- [x] 顾客八帧动画、随身商品标记、减少动态效果冻结和五人表现预算
- [x] Release 全量测试、构建、漏洞审计和变更范围格式检查
- [x] GitHub PR、Windows 发布门禁、正式 Release 与隔离工作区清理
- **Status:** complete

### Phase 21: 0.1.14 人物动画与资源整治

- [x] 收银员、补货员、顾客与街区路人统一为 24 帧表现时间轴
- [x] 8 个关键姿势去重存储，位置仍在每个表现帧推进
- [x] 审计并清除人物断裂像素，运行时加载拒绝空白、断裂和贴边格
- [x] 图集转为索引色并从 57,191 字节降至 16,623 字节
- [x] Release 全量测试、构建和便携包真实启动
- **Status:** complete
...
- [x] 独立审阅、完整 Release 门禁、GitHub PR/标签和工作区清理
- **Status:** complete
